...
{
    public class VisiteViewModel : IVisiteViewModel, INotifyPropertyChanged
    {
        private HoroazhonContext _horoazhonContext;
        private Visite _consultationSelected = new();
        private RendezVous? _rdvSelected;

        public VisiteViewModel(RendezVous? elem = null)
        {
            ...
            if (elem != null)
            {
                ...
                else
                {
                    ...
                    var idMedecin = Services.User.UserService.AUser?.Agent?.Idpers;

                    if (idMedecin != null)
                    {
                        ReloadConsultationSelected(new RendezVous()
                        {
                    ...
                    else
                    {
                        ...
                        _consultationSelected = new Visite()
                        {
               ...
               /* ReloadConsultationSelected(new RendezVous()
                {
                    Idpersmedecin = Services.User.User.AUser.Agent.Idpers
                }
        ...
        public List<Visite>? Consultations { get; set; } = new();
        public List<RendezVous>? Rdvs { get; set; } = new();
        #endregion
        ...
        public void ReloadConsultationSelected(RendezVous? elem)
        {
            ...
            try
            {
                _consultationSelected.RendezVous = elem;
                ...
                ConsultationMedecinNom = $"Dr. {_consultationSelected.RendezVous.IdpersmedecinNavigation.IdpersNavigation.Nompers}";
                ConsultationClientNom = $"{_consultationSelected.RendezVous.IdpersClientNavigation!.IdpersNavigation.Nompers}";
                _consultationSelected.Motifcons = _consultationSelected.RendezVous.Commentairerdv;
                ConsultationMotif = _consultationSelected.Motifcons;

                ConsultationSelected = _consultationSelected;

                _horoazhonContext = new HoroazhonContext();
                Consultations = _horoazhonContext.Consultations
                    .Where(x => x.Idpersmedecin == _consultationSelected!.Idpersmedecin &&
                                x.RendezVous.IdpersClient == _consultationSelected.RendezVous.IdpersClient)
                    .ToList();
            }
            catch (Exception ex)
            {
        ...
        private void ActionConsultationSave()
        {
            try
            {
                ...
                if (_consultationSelected.Idcons == 0)
                    _horoazhonContext.Consultations.Add(_consultationSelected);
                else
                    _horoazhonContext.Consultations.Update(_consultationSelected);
