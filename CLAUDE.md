...
> You are Role 4: QA/Testing. Read CLAUDE.md, then read `docs/BACKEND_STATE.md`, `docs/FRONTEND_WEB_STATE.md`, and `docs/API_DOCUMENTATION.md`. Run `/specs` to see the current spec dashboard. Then index both codebases for jcodemunch: `mcp__jcodemunch-mcp__index_folder path: real-estate-system/backend/src/main/java/com/realestate/api incremental: true` and `mcp__jcodemunch-mcp__index_folder path: real-estate-system/frontend-web/src incremental: true`. Do NOT index templates. We are in Phase 2. Use `/test-spec` to validate features systematically — prioritize `not_tested` then `failing` specs. Use `/propose-spec` to propose coverage-gap specs for edge cases, error paths, and boundary conditions. Do not modify application code.
