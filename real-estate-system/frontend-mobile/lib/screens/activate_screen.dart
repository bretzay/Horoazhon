...
class _ActivateScreenState extends State<ActivateScreen> {
  ...
  Future<void> _checkToken() async {
    ...
    try {
      final result = await _api.checkActivationStatus(_tokenController.text.trim());
      if (mounted) {
        setState(() {
          _tokenValid = result['valid'] == true;
          if (!_tokenValid) _errorMessage = 'Lien d\'activation invalide ou expiré';
      ...
      if (mounted) {
        setState(() {
          _errorMessage = 'Lien d\'activation invalide ou expiré';
          _tokenValid = false;
          _isCheckingToken = false;
        });
      }
  ...
  Future<void> _activate() async {
    ...
    setState(() { _isActivating = true; _errorMessage = null; });
    try {
      await _api.activateAccount(_tokenController.text.trim(), _passwordController.text);
      if (mounted) {
        setState(() {
          _successMessage = 'Compte activé avec succès ! Vous pouvez maintenant vous connecter.';
          _isActivating = false;
        });
      }
    } catch (e) {
      if (mounted) {
        setState(() {
          _errorMessage = 'Erreur lors de l\'activation';
          _isActivating = false;
        });
      }
    }
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              child: const Icon(Icons.key, size: 32, color: AppColors.white),
            ),
            const SizedBox(height: AppSpacing.space4),
            Text('Activation du compte', style: AppTextStyles.textXl.w700),
                  ...
                  child: Form(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Token
                        Text('Token d\'activation', style: AppTextStyles.textMd.w500),
                        ...
                        if (_tokenValid) ...[
                          ...
                          Text('Confirmer le mot de passe', style: AppTextStyles.textMd.w500),
                          const SizedBox(height: AppSpacing.labelInputGap),
                          TextFormField(
